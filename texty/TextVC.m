...
- (void) signal 								{
	if (need_to_autosave) 
		if (time(NULL) - autosave_ts > Preferences.defaultAutoSaveInterval) {
			if (s.temporary)	[s migrate:s.fileURL withString:text.string autosaving:YES];
			else 					[s autosave:NO];
...
-   (id) initWithFrame:(NSRect) frame 	{
	...
	m.postsBoundsChangedNotifications = YES;
	[NSNotificationCenter.defaultCenter addObserver:self selector:@selector(boundsDidChange:) name:NSViewBoundsDidChangeNotification object:m]; 
 	[NSNotificationCenter.defaultCenter addObserver:self selector:@selector(syntax_reload) name:@"AppDelegateSet" object:nil]; 
	return self;
}
- (NSProgressIndicator*)working {

	NSObject* del = [[NSApplication sharedApplication] delegate];
	return (del) ? [del valueForKey:@"working"] : nil;
}

- (void) responder 							{	text.selectedRange = NSMakeRange(0, 0);	[scroll becomeFirstResponder];	}
- (void) syntax_reload	 					{
	
	self.working ? [self.working startAnimation:nil] : nil;
...
- (BOOL) save 									{	return [self saveAs:s.fileURL];	}
- (void) revertToSaved	 					{	[text setString:s.data];			}
- (BOOL) is_modified 						{	return ![text.string isEqualToString:s.data];	}
...
- (void) boundsDidChange:(id)noti 		{	[text delayedParse];	}
- (void) run_diff_against:(NSURL*)b 	{	[self run:[m_exec diff:(NSURL*)s.fileURL against:b] withTimeout:0];	}
- (void) run_self 							{
...
- (void) run: (NSString*) cmd withTimeout:(int) timeout {
	if (!ewc) self.ewc = ExecuteWC.alloc.init;
	[ewc execute:cmd withTimeout:timeout];
	[ewc.window makeKeyAndOrderFront:nil];
}
- (void) close 								{	[s close];	[ewc.e terminate];	self.ewc = nil;	}
